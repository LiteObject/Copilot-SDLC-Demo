...
| **Shared rules** | `docs/portable-agent-contract.md` is canonical; generated Copilot and optional generic adapters expose it alongside `.instructions.md` files |
...
consuming repository. The generic root adapter is generated by the installer
when `--agent-surface generic` or `--agent-surface all` is selected:
...
template/
       ...
       base/                                 # always-installed target-relative files
              ...
              scripts/scope-audit.*
              scripts/generate-agent-surfaces.*
              scripts/validate-agent-surfaces.*
       AGENTS.md                              # optional generated generic adapter
       extensions/
...
- `docs/portable-agent-contract.md`: editor-independent source of truth for
       phases, state schema, gates, task IDs, permissions, prohibited actions,
       evidence, and escalation.
- `copilot-instructions.md`: generated VS Code adapter that points to the
       portable contract and adds Copilot-specific delegation and approval UX.
- `AGENTS.md`: optional generated generic adapter containing the same contract.
       Its template version and source hash make stale projections detectable.
- `scripts/validate-agent-surfaces.*`: compares selected adapters and writes
...
- **Portable agent surface:** the generic `AGENTS.md` projection plus the
       checked-in scripts and evidence can run without VS Code or Copilot. Native
       subagent delegation, custom-agent selection, and interactive approval UX are
       Copilot-specific and require a human or another surface's equivalent.
