...
for field in policy_path permissions_path threat_model_path evaluation_plan_path evaluation_scenarios_path ledger_path evaluation_evidence_path approved_providers approved_models approved_tenants permitted_repositories allowed_data_classifications prohibited_inputs tool_allowlist mcp_server_allowlist network_destination_allowlist credential_scope_allowlist phase_tool_grants restricted_actions approval_required_actions sandbox_type untrusted_input_policy evaluation_task; do require_value "$field"; done
...
for field in approved_providers approved_models approved_tenants permitted_repositories allowed_data_classifications prohibited_inputs tool_allowlist mcp_server_allowlist network_destination_allowlist credential_scope_allowlist phase_tool_grants restricted_actions approval_required_actions; do value="$(get_value "$field")"; [[ "$value" == \[*\] ]] || add_error "ai_governance.$field must be an inline YAML list."; done
