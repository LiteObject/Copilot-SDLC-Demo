...
if (( RECORD_SPEC == 1 )); then
    ...
    if [[ -n "$FEATURE_ID" ]]; then
        set_spec_field gate_ai_lifecycle_feature_id "\"$FEATURE_ID\""
        set_spec_field gate_ai_lifecycle_spec_path "\"$SPEC_RELATIVE_PATH\""
    fi
fi
