...
get_value() {
    local value
    if value="$(canonical_contract_query "$1" scalar 2>/dev/null)"; then printf '%s' "$value"; else printf '%s' "${2-}"; fi
}
get_list() {
    LIST_RESULT=()
    local item
    while IFS= read -r -d '' item; do LIST_RESULT+=("$item"); done < <(canonical_contract_query "$1" nul 2>/dev/null || true)
}
