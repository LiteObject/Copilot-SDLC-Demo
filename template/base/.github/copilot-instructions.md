...
- Shared configuration, manifests, lockfiles, and workflows require an exact `approved_shared_files` record in the selected feature spec; do not treat `planned_files` alone as shared-file approval.
- Feature task graphs live at `docs/specs/<feature-id>/tasks.json`. Validate them with `python scripts/task-graph.py validate --feature-id <id>`; use `-TaskId` / `--task-id` when recording verification evidence. A task cannot satisfy REVIEW or DONE without the required status and current evidence.
...
- `task-graph.py` — validates per-feature task DAGs, mappings, task scope, statuses, and revision-bound evidence; it also records verification evidence and prints a graph summary.
- `task-graph.py` — validates per-feature `tasks.json` DAGs, requirement and acceptance mappings, verification task IDs, scope paths, statuses, and revision-bound task evidence. A newly scaffolded graph is intentionally incomplete until the Architect populates it.
